...
local function tiledProperties(properties)
    if (#properties > 0) and properties[1].name and properties[1].value then
        ...
        for i = 1, #properties do
            if translate then
                if properties[i].type == "string" and (properties[i].name == "text") and
                    (not tonumber(properties[i].value)) then
                    if translate then
...
local function decodeTiledColor(hex)
    ...
    local a, r, g, b = hexToFloat(hex:sub(1, 2)), hexToFloat(hex:sub(3, 4)), hexToFloat(hex:sub(5, 6)),
                ...
                for tx = 0, data.width - 1 do
                    ...
                    if gid then
                        ...
                        if animation then
                            ...
                            image = sheet and display.newImage(objectGroup, sheet, gid, 0, 0) or
                ...
                if object.gid then
                    ...
                    if gid then
                        ...
                        if animation then
                            ...
                            image = sheet and display.newImageRect(objectGroup, sheet, gid, object.width, object.height)
                                or
                                display.newImageRect(objectGroup, path and path.normalize(dir .. gid) or (dir .. gid),
                        ...
                        if autoShape then
                            if not sheet then
                                object.properties.outline = graphics.newOutline(autoShape,
